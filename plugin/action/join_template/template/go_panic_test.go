...
func BenchmarkPanicStartMixedRes(b *testing.B) {
	...
	b.Run("regexp", func(b *testing.B) {
		for i := 0; i < b.N; i++ {
			for _, line := range lines {
				GoPanicStartRe.MatchString(line)
			}
		}
	})
}
...
func BenchmarkPanicContinueMixedRes(b *testing.B) {
	...
	b.Run("regexp", func(b *testing.B) {
		for i := 0; i < b.N; i++ {
			for _, line := range lines {
				GoPanicContinueRe.MatchString(line)
			}
		}
	})
}
...
func TestPanicSameResults(t *testing.T) {
	...
	for _, line := range lines {
		require.Equal(t, GoPanicStartRe.MatchString(line), GoPanicStartCheck(line), line)
		require.Equal(t, GoPanicContinueRe.MatchString(line), GoPanicContinueCheck(line), line)
	}
}
