...
func TestRename(t *testing.T) {
	...
	input.In(0, "test.log", test.NewOffset(0), []byte(`{"field_1":"value_1"}`))
	input.In(0, "test.log", test.NewOffset(0), []byte(`{"field_2":"value_2"}`))
	input.In(0, "test.log", test.NewOffset(0), []byte(`{"field_3":"value_3"}`))
	input.In(0, "test.log", test.NewOffset(0), []byte(`{"field_4":{"field_5":"value_5"}}`))
	input.In(0, "test.log", test.NewOffset(0), []byte(`{"k8s_node_label_topology.kubernetes.io/zone":"value_6"}`))
...
func TestRenamingSequence(t *testing.T) {
	...
	input.In(0, "test.log", test.NewOffset(0), []byte(`{"key1":"value_1"}`))
