...
func callParseRegexAll(t *testing.T, value, pattern string, named map[string]core.Value) []string {
	t.Helper()

	got, err := callFn(parseRegexAll{}, []core.Value{
...
func TestParseRegexAll(t *testing.T) {
	t.Parallel()

	t.Run("whole_matches", func(t *testing.T) {
		t.Parallel()
		assert.Equal(t, []string{"re1", "re2", "re3", "re4"},
			callParseRegexAll(t, "re1 re2 re3 re4", `re\d+`, nil))
	})

	t.Run("capture_group", func(t *testing.T) {
		t.Parallel()
		assert.Equal(t, []string{"1", "2"},
			callParseRegexAll(t, "re1 re2", `re(\d+)`, map[string]core.Value{"group": core.IntegerValue{V: 1}}))
	})
	...
	t.Run("limit", func(t *testing.T) {
		...
		assert.Equal(t, []string{"re1", "re2"},
			callParseRegexAll(t, "re1 re2 re3 re4", `re\d+`, map[string]core.Value{"limit": core.IntegerValue{V: 2}}),
			"a positive limit caps the number of matches")

		assert.Empty(t, callParseRegexAll(t, "re1 re2", `re\d+`, map[string]core.Value{"limit": core.IntegerValue{V: 0}}),
			"limit 0 extracts nothing, as in the modify plugin")

		assert.Equal(t, []string{"re1", "re2"},
			callParseRegexAll(t, "re1 re2", `re\d+`, map[string]core.Value{"limit": core.IntegerValue{V: -5}}),
			"any negative limit means all matches")
	})

	t.Run("no_match", func(t *testing.T) {
		t.Parallel()
		assert.Empty(t, callParseRegexAll(t, "nothing here", `re\d+`, nil))
		assert.Empty(t, callParseRegexAll(t, "", `re\d+`, nil), "empty input")
	})
	...
	t.Run("group_out_of_range", func(t *testing.T) {
		...
		call := func(group int64) error {
			_, err := callFn(parseRegexAll{}, []core.Value{
