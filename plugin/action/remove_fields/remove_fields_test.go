...
func TestRemoveFields(t *testing.T) {
	...
	input.In(0, "test.log", 0, []byte(`{"field_1":"value_1","a":"b"}`), pipeline.NewMeta())
	input.In(0, "test.log", 0, []byte(`{"field_2":"value_2","b":"c"}`), pipeline.NewMeta())
	input.In(0, "test.log", 0, []byte(`{"field_3":"value_3","a":"b"}`), pipeline.NewMeta())
