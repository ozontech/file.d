...
func TestRemoveNestedFields(t *testing.T) {
	...
	input.In(0, "test.log", test.NewOffset(0), []byte(`{"a":"some"}`))
	input.In(0, "test.log", test.NewOffset(0), []byte(`{"a":{"b":"deleted"}}`))
	input.In(0, "test.log", test.NewOffset(0), []byte(`{"a":{"b":{"c":["deleted"]},"d":"saved"}}`))
