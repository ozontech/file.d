			...
			for {
				...
				for i := int64(0); i < read; i++ {
					...
					if len(accumBuffer) != 0 {
						...
						head.Push(jobProvider, job.inode, file.Name(), offset, accumulated+i+1, accumBuffer)
					} else {
						head.Push(jobProvider, job.inode, file.Name(), offset, i+1, readBuffer[processed:i])
					}
					accumBuffer = accumBuffer[:0]
