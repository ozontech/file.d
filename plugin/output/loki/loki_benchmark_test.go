...
// BenchmarkLokiEventProcessing benchmarks individual event processing
func BenchmarkLokiEventProcessing(b *testing.B) {
	eventCounts := []int{1, 10, 100, 1000}
	...
	for _, eventCount := range eventCounts {
		b.Run(fmt.Sprintf("Events_%d", eventCount), func(b *testing.B) {
			...
			for i := 0; i < b.N; i++ {
				for _, eventData := range events {
					event := createBenchmarkEvent(eventData)
					_ = event // Use the event to avoid optimization
				}
			}
		})
	}
...
// BenchmarkLokiMemoryUsage benchmarks memory usage patterns
func BenchmarkLokiMemoryUsage(b *testing.B) {
...
// BenchmarkLokiLabelProcessing benchmarks label processing performance
func BenchmarkLokiLabelProcessing(b *testing.B) {
	labelCounts := []int{1, 5, 10, 20, 50}
	...
	for _, labelCount := range labelCounts {
		b.Run(fmt.Sprintf("Labels_%d", labelCount), func(b *testing.B) {
			...
			for i := 0; i < b.N; i++ {
				// Convert labels to string (this is what happens in the plugin)
				var result string
				for k, v := range labels {
					result += k + "=" + v + ","
				}
				_ = result // Use the result to avoid optimization
			}
		})
	}
}

// BenchmarkLokiTimestampProcessing benchmarks timestamp processing
func BenchmarkLokiTimestampProcessing(b *testing.B) {
	plugin := &Plugin{}

	// Generate various timestamp formats
	timestamps := []string{
		"1700000000000000000", // Valid UnixNano
		"1609459200123456789", // Valid UnixNano
		"1640995200000000000", // Valid UnixNano
		"invalid_timestamp",   // Invalid
		"1234567890",          // Invalid (too short)
		"9999999999999999999", // Invalid (too large)
	}
