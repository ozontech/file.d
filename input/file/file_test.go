...
func startController(persistenceMode string, enableEventLog bool, config *Config) (*pipeline.SplitController, *FilePlugin) {
	...
	controller.SetInputPlugin(&pipeline.PluginWithConfig{Instance: inputPlugin, Config: config})
	controller.Start()

	return controller, inputPlugin.(*FilePlugin)
}
