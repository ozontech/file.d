...
> ⚠ Though we use it in production, `it's still less than v1.0.0`. Please, test your pipelines carefully on dev/stage environments.  

## Motivation
Well, we already have several similar tools: vector, filebeat, logstash, fluend-d, fluent-bit, etc.

Performance tests state that the best ones achieve around 100MB/sec throughput. 
...
Throughput on the production server to be filled.  
